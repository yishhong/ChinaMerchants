...
//    [self showHint:[NSString stringWithFormat:@"新用户加入%@",aChatroom.subject] yOffset:-frame.size.height + KHintAdjustY];
}
                              ...
                              reason:(EMChatroomBeKickedReason)aReason
{
    if ([_conversation.conversationId isEqualToString:aChatroom.chatroomId])
    {
        ...
        _isKicked = YES;
        CGRect frame = self.chatToolbar.frame;
#warning 聊天室退出聊天室 目前调用的后台接口 所以这里是被踢出的
//        [self showHint:[NSString stringWithFormat:@"你已退出聊天室！"] yOffset:-frame.size.height + KHintAdjustY];
